...
$arrmsg = [];
$sqlmessage="SELECT message.*,customer.customer_name,painter.name FROM message left JOIN customer ON customer.customer_id=message.sender_id LEFT JOIN painter ON painter.painter_id=message.receiver_id WHERE message.sender_id='$_POST[customer_id]' AND message.receiver_id='$_POST[painter_id]' AND (message.message_type='Customer2Painter' OR message.message_type='Painter2Customer') ORDER BY message_date_time ASC";
